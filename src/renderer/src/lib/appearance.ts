 ...
 * - 颜色主题（colorTheme）：12 套 NewMax 预设色板（azure/claude/dusk/elegant/
 *   image-wallpaper/luxury/nature/ocean/professional/retro/snow-cinnabar/vibrant）
 *   —— 每套在 themes.css 中定义 html[data-theme="X"]（浅）+ html[data-theme="X"].dark（深）。
 *   主强调色由 --ds-brand-primary 派生（按钮/开关/聚焦环/选中态直接用品牌色），
 *   与 NewMax「--ds-action-primary = var(--ds-brand-primary)」一致。
...
/** NewMax 颜色主题（整面色板）：每个主题对应 themes.css 中的 html[data-theme]，
 *  定义品牌色 / 强调色 / 表面 / 文字基调，主强调色由 --ds-brand-primary 派生。 */
...
export async function loadAppearance(): Promise<Appearance> {
  ...
  return {
    ...
    colorTheme: validTheme in Object.fromEntries(COLOR_THEMES.map((s) => [s.key, 1])) || validTheme === 'none' ? validTheme : DEFAULT_APPEARANCE.colorTheme,
...
export function applyAppearance(a: Appearance): void {
  ...
  // 颜色主题（整面色板 + 主强调色 --ds-brand-primary，由 themes.css 的 html[data-theme] 决定）
  if (a.colorTheme === 'none') root.removeAttribute('data-theme')
  else root.setAttribute('data-theme', a.colorTheme)
